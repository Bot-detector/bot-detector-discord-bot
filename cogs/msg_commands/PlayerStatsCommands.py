...
class PlayerStatsCommands(utils.CommonCog, name="Player Stats Commands"):
    ...
    async def lookup(self, ctx, *, username):
        ...
        boss_embed = discord.Embed(
            title=f"{username}'s Boss/Minigame KC",
            description="OSRS Hiscores Lookup",
            color=0x00FF00,
        )
                ...
                try:
                    ...
                    try:
                        ...
                        return await ctx.reply(
                            f"I cannot locate a hiscores entry for {username}."
                        )
    ...
    async def kc(self, ctx, *, player_name=None):
        if not player_name:
            ...
            accounts = await discord_processing.get_linked_accounts(
            ...
            embed = discord.Embed(
                title=f"{ctx.author.display_name}'s Stats", color=0x00FF00
            )
        ...
        if await checks.check_patron(ctx):
            patron = True
            url = (
                f"https://www.osrsbotdetector.com/api/stats/contributions?token={token}"
            )
        else:
            patron = False
    ...
    async def assemble_kc_embed(self, embed: discord.Embed, js: dict, is_patron: bool):
        ...
        embed.add_field(
            name="Sightings Submitted:", value=f"{total_reports:,d}", inline=False
        )
        embed.add_field(
        ...
        if feedback > 0:
            embed.add_field(
                name="Feedback Submitted:", value=f"{feedback:,d}", inline=False
            )

        if is_patron:
            embed.add_field(
                name="Total XP Removed:",
                value=f"{js['total']['total_xp_removed']:,.0f}",
                inline=False,
            )
    ...
    @commands.command(
        aliases=["updaterank", "rankme", "lvlup", "levelup"],
        description=help_messages.rankup_help_msg,
    )
    async def rankup(self, ctx):
        member = ctx.author
        linkedAccounts = await discord_processing.get_linked_accounts(
            self.bot.session, member.id, token
        )
        ...
        for r in member.roles:
            ...
            verified_role = discord.utils.find(
                lambda r: r.id == roles.special_roles["Discord-RSN Linked"]["role_id"],
                member.guild.roles,
            )
            await member.add_roles(verified_role)

        current_role = discord.utils.find(
            lambda r: "Bot Hunter" in r.name, member.roles
        )
        new_role, current_amount, next_role_amount = await roles.get_bot_hunter_role(
            self.bot.session, linkedAccounts, member
        )
    ...
    async def predict(self, ctx, *, player_name):
        ...
        async with self.bot.session.get(
            f"https://www.osrsbotdetector.com/api/v1-bot/site/prediction/{player_name}"
        ) as r:
            if r.status != 200:
                return await ctx.reply(
        ...
        if secondaries is not None:
            ...
            secondaries = {
                k: v
                for k, v in sorted(
                    secondaries.items(), key=lambda item: item[1], reverse=True
                )
            }
    ...
    async def export_bans(self, ctx, file_type):
        ...
        info_embed.set_thumbnail(
            url="https://i.pinimg.com/originals/c1/b4/ee/c1b4ee02fc804310213be0ca427af5f8.png"
        )
        ...
        async with self.bot.session.post(
            ...
            await ctx.author.send(
                f"Here's your link! https://www.osrsbotdetector.com/api/discord/download_export/{res_data['url']}"
            )
            ...
            try:
                ...
                print(
                    "Original command message was deleted, so there is nothing to reply to. Move along!"
                )

            return

    @commands.command(
        aliases=["excelbans", "bansexport"], description=help_messages.excelban_help_msg
    )
    ...
    async def equip(self, ctx, *, player_name):
        ...
        req_payload = {"player_name": player_name}
    ...
    @commands.command(
        aliases=["gainz", "xpdiff", "xpgains", "gains"],
        description=help_messages.xpgain_help_msg,
    )
    async def xpgain(self, ctx, *, player_name):
        ...
        async with self.bot.session.post(
            ...
            if r.status == 200:
                ...
                await ctx.reply(
                    f"I couldn't locate {player_name}'s hiscores gains. Sorry!"
                )
    ...
    async def pwned(self, ctx, *, player_name):
        ...
        if is_pwned == "ERROR":
            await ctx.reply(
                f"I couldn't get the ban information for {player_name}. I might be rate-limited, or the RS servers may be having issues."
            )
        if is_pwned:
    ...
    @commands.command(
        aliases=["ban_list_check", "ban_check", "bans_check", "check_bans"]
    )
    async def ban_list(self, ctx, paste_url):
        ...
        names_list = [
            name for name in names_list if string_processing.is_valid_rsn(name)
        ]

        player_data = await discord_processing.get_players(
            session=self.bot.session, player_names=names_list, token=token
        )
